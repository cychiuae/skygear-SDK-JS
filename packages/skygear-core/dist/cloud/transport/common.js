...
var SkygearResponse = exports.SkygearResponse = function () {
  ...
  _createClass(SkygearResponse, [{
    ...
    key: 'toResultJSON',
    value: function toResultJSON() {
      var _this = this;

      var header = {};
      var status = this.statusCode || 200;
      var body = b64EncodeUnicode(this.body);

      Object.keys(this.headers).forEach(function (perKey) {
        header[perKey] = _this.headers[perKey];
      });
  ...
  _createClass(CommonTransport, [{
    ...
    value: function eventHandler(payload) {
      ...
      var funcPromises = funcList.map(function (eachFunc) {
        return _this2._promisify(eachFunc, payload.param);
      });
    ...
    value: function providerHandler(payload) {
      ...
      return this._promisify(provider.handleAction.bind(provider), param.action, param).then(function (result) {
        return { result: result };
      });
    }
  }]);
